...
fn main() {
    ...
    if let Some(pcap_path) = args.pcap {
        ...
        while let Ok(packet) = cap.next_packet() {
            ...
            if frame_control == 0x80 { // 0x80 = Beacon (Type 0, Subtype 8)
...
fn analyser_tlv(mut data: &[u8], mac_addr: String) {
    ...
    while data.len() >= 2 {
        ...
        if data.len() < 2 + tag_len { break; } // sécurité
        
        let tag_value = &data[2..2 + tag_len]; //parcourt les valeurs des données
        ...
        match tag_type {
            ...
            0xdd => { // Vendor Specific
                // Si on trouve ce tag, on affiche une alerte
                println!("!!! DRONE POTENTIEL DÉTECTÉ !!!");
                println!("MAC: {} | Tag DroneID (0xdd) trouvé", mac_addr);
            },

            _ => {} // On ignore les autres types
        }

        // On avance dans le tableau d'octets pour lire le tag suivant
        data = &data[2 + tag_len..];
    }

    // Affichage final pour chaque Beacon
    println!("[BEACON] MAC: {} | SSID: {}", mac_addr, ssid);
}